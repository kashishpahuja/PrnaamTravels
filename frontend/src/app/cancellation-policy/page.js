...
export default function PolicyPage({ params }) {
  ...
  // For this example, let's default to 'cancellation-policy' if params isn't used
  const slug = params?.slug || "cancellation-policy";
  const data = POLICY_CONTENT[slug] || POLICY_CONTENT["cancellation-policy"];
